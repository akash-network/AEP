...
The ideal user experience is one where a user deploying via Akash Console receives multiple bids from each provider that bids on their workload - potentially as a table of bids with multiple columns. One set of bids is for "on-demand" lease while the others are for different lengths of "reservations" (say 3months, 6months, 9months, 12 months). The longer the reservation (commitment) the lower the price.

Choosing a given reserved price should do two things. First it should check and attempt to escrow funds from the users wallet to cover the entire duration of the reservation for the requested resources. Second it should allocate a portion of the provider (likely one or more nodes of the provider) to that specific tenant - this will need to be done by dynamically updating a whitelist that can be applied per node of the provider.

...
3. Enabling Auto Credit Reload requires them to specify two values:
   ...
   - A "Reload Threshold" (this is the condition that will trigger the service to perform a reload). There will be a minimum amount for this also (say $5)
4. The service will monitor the account balance and when the balance is at or below the Reload Threshold it will trigger a credit purchase to bring the balance up to the Reload Balance. For example if the user sets a Reload Balance of $100 and a Threshold of $20, a credit purchase of $80 will automatically be performed when the account balance drops to $20.
5. The user will have an option to be notified (via email) when the Auto Credit Reload is performed. 
6. If the Auto Reload fails (due to an invalid credit card or other issue) the user will be notified. Not addressing the issue may result in the balance going to $0 - which will over time cause deployments to run out of funds and shut down.
